...
class FirebaseManager:
    ...
    def is_state_locked(self, collection): return len(self.db.collection(collection).limit(1).get()) > 0
...
# LOGIC
# =========================
def get_stake(collection, base_stake):
    last = firebase_manager.get_last_resolved(collection)
    if not last or last.get('outcome') == 'win': return base_stake, 1
...
def process_match(match):
    ...
    if '1ST' in status and min_elapsed in OVER05_TRIGGER_MINUTES and score == '0-0' and not OVER05_TRACKED_MATCHES[fid]['placed']:
        if not firebase_manager.is_state_locked('unresolved_over05_bets'):
            stake, seq = get_stake('resolved_over05_bets', OVER05_STAKE)
            data = {'match_name': match_name, 'league': league, 'country': country, 'trigger_score': score, 'stake': stake, 'match_sequence': seq}
    ...
    if '1ST' in status and min_elapsed in MINUTES_REGULAR_BET and not LOCAL_TRACKED_MATCHES[fid]['placed']:
        if not firebase_manager.is_state_locked('unresolved_bets') and score in ['1-1', '2-2', '3-3']:
            stake, seq = get_stake('resolved_bets', ORIGINAL_STAKE)
            data = {'match_name': match_name, 'league': league, 'country': country, '36_score': score, 'stake': stake, 'match_sequence': seq}
