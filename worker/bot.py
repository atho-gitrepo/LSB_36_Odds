...
OVER05_TRIGGER_MINUTE = [20,21,22]
OVER05_STAKE = 10.0
...
def send_telegram(msg):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.warning(f"⚠️ Telegram send failed with status: {response.status_code}")
    except Exception as e:
...
def calculate_over05_stake():
    ...
    if not last or last.get('outcome') == 'win':
        logger.info(f"📊 Over 0.5: No previous or last was win - using original stake ${OVER05_STAKE}")
        return OVER05_STAKE, 1
    seq = last.get('match_sequence', 1)
    if seq < MAX_CHASE_LEVEL:
        new_stake = float(OVER05_STAKE * (2**seq))
        logger.info(f"📊 Over 0.5: Chase level {seq} -> next stake ${new_stake:.2f}")
        return new_stake, seq + 1
    logger.info(f"📊 Over 0.5: Max chase level reached - resetting to original stake ${OVER05_STAKE}")
    return OVER05_STAKE, 1
...
def process_match(match):
    ...
    logger.debug(f"📊 Processing: {match_name} | Minute: {min_elapsed} | Status: {status} | Score: {score}")
    ...
    # --- OVER 0.5 TRIGGER: Score 0-0 at trigger minutes (20,21,22) ---
    if (status == '1ST_HALF' or '1ST' in status) and \
       min_elapsed in OVER05_TRIGGER_MINUTE and \
       score == '0-0' and \
       not over05_state['bet_placed'] and \
       not firebase_manager.is_over05_state_locked():
        
        logger.info(f"⚽ Over 0.5 trigger condition met for {match_name} at {min_elapsed}'")
        ...
        logger.info(f"✅ Over 0.5 bet placed notification sent: {match_name}")
        
        over05_state['bet_placed'] = True
        OVER05_TRACKED_MATCHES[fid] = over05_state
        
        logger.info(f"🚀 Over 0.5 bet placed: {match_name} | {score} at {min_elapsed}' | Stake: ${stake:.2f} | Sequence: {seq}")

    # --- OVER 0.5 CHECK: At halftime ---
    if 'HALFTIME' in status and over05_state['bet_placed']:
        logger.info(f"🔍 Checking Over 0.5 result for {match_name} at halftime")
        ...
        if unresolved_over05:
            ...
            result_message = (
                ...
                f"Bet placed at {unresolved_over05.get('trigger_minute', '?')}' with score 0-0\n"
                f"Stake: ${unresolved_over05.get('stake', 0):.2f}"
            )
            send_telegram(result_message)
            logger.info(f"🔍 Over 0.5 result: {outcome} | {match_name} | Score: {score} | Goals: {total_goals}")
    ...
    if '1ST' in status and min_elapsed in MINUTES_REGULAR_BET and not state['bet_placed']:
        if not firebase_manager.is_state_locked():
            if score in ['1-1', '2-2', '2-1','2-0']:
                ...
                send_telegram(message)
                logger.info(f"✅ Regular bet placed notification sent: {match_name}")
    ...
    # =========================
        ...
        logger.info(f"🔍 Checking Regular bet result for {match_name} at halftime")
        ...
        if unresolved:
            ...
            send_telegram(result_message)
            logger.info(f"🔍 Regular bet result: {outcome} | {match_name} | Score: {score}")
...
def cleanup_old_entries():
    ...
    for fid in list(LOCAL_TRACKED_MATCHES.keys()):
        if current_time - LOCAL_TRACKED_MATCHES[fid]['last_seen'] > 600:  # 10 minutes
            logger.debug(f"Cleaning up old regular match: {fid}")
            LOCAL_TRACKED_MATCHES.pop(fid, None)
    ...
    for fid in list(OVER05_TRACKED_MATCHES.keys()):
        if current_time - OVER05_TRACKED_MATCHES[fid]['last_seen'] > 600:
            logger.debug(f"Cleaning up old over 0.5 match: {fid}")
            OVER05_TRACKED_MATCHES.pop(fid, None)
...
def initialize_bot_services():
    ...
    try:
        ...
        logger.error(f"❌ Failed to initialize Sofascore: {e}")
        return False
...
def send_status_report():
    ...
    send_telegram(msg)
    logger.info(f"Status report sent - Regular: {regular_tracked}, Over0.5: {over05_tracked}")
...
def run_bot_cycle():
    ...
    try:
        ...
        logger.info(f"🔄 Scanning {len(events)} live matches | Regular tracked: {len(LOCAL_TRACKED_MATCHES)} | Over0.5 tracked: {len(OVER05_TRACKED_MATCHES)}")
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            # Send status report every 30 minutes (approximately 19 cycles with 95s sleep)
